    ...
    public async Task ExecutarAsync(Ator ator, Guid ordemServicoId, IOrdemServicoGateway gateway, IOperacaoOrdemServicoPresenter presenter, IAppLogger logger)
    {
        try
        {
            if (!ator.PodeGerenciarOrdemServico())
                throw new DomainException("Acesso negado. Apenas administradores podem finalizar a execução de ordens de serviço.", ErrorType.NotAllowed, "Acesso negado para finalizar execução da ordem de serviço {OrdemServicoId} pelo usuário ator {Ator_UsuarioId}", ordemServicoId, ator.UsuarioId);
