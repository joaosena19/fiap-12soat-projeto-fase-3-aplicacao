...
{
    public async Task ExecutarAsync(Ator ator, Guid id, IItemEstoqueGateway gateway, IBuscarItemEstoquePorIdPresenter presenter, IAppLogger logger)
    {
        try
        {
            if (!ator.PodeGerenciarEstoque())
                throw new DomainException("Acesso negado. Apenas administradores podem buscar estoque.", ErrorType.NotAllowed, "Acesso negado para buscar item de estoque para usuário {Ator_UsuarioId}", ator.UsuarioId);
